...
// const로 키워드가 있어 리스트의 내용을 변경하지 않음을 보장한다
{
	...
	while (current != nullptr) // current가 nullptr이 아닐 때 즉 리스트 끝에 도달하지 않았을 동안 반볻한다. 
...
// 장점 :
// 순서를 바꾸기 매우 쉽다.
// 중간에서 새로운 자료를 넣거나 빼기도 싶다
// 크기가 고정되지 않는다(배열처럼 정하지 않아도 된다)
// 데이터 저장을 위한 메모리를 공간이 필요할 때마다 동적으로 만들어 쉽게 추가할 수 있다.

// 단점 :
// 임의 접근 불가 - 배열처럼 list[3]으로 바로 접근이 불가능하다. -> 항상 head부터 하나씩 따라가야한다. 시간 복잡도 O(n)
// 이전 노드로 이동 불가 
// 삭제 시 이전 노드 필요
// 포인터 메모리 소모 - 각 노드마다 next 포인터가 필요하다.
// 복잡한 삽입 삭제 조건 처리 : 코드가 복잡해지고 지저분해질 수 있다.
// 순환 구조 없음 - 끝에 도달하면 다시 시작하려면 다시 head부터 가야한다.-> 비효율적이다.
// 탐색이 느리다. 
